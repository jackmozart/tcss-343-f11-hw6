...
public class DijkstraMapTest {
	...
	@Test public void testComplex() {
		...
		assertEquals("Wrong cost for a", 0, 
				map.getNode(my_vertex_map.get('a')).getCost());
		assertEquals("Wrong cost for b", 3, 
				map.getNode(my_vertex_map.get('b')).getCost());
		assertEquals("Wrong cost for c", 4, 
				map.getNode(my_vertex_map.get('c')).getCost());
		assertEquals("Wrong cost for d", 8, 
				map.getNode(my_vertex_map.get('d')).getCost());
		assertEquals("Wrong cost for e", 10, 
				map.getNode(my_vertex_map.get('e')).getCost());
		assertEquals("Wrong cost for f", 2, 
				map.getNode(my_vertex_map.get('f')).getCost());
		assertEquals("Wrong cost for g", 2, 
				map.getNode(my_vertex_map.get('g')).getCost());
		assertEquals("Wrong cost for h", 5, 
				map.getNode(my_vertex_map.get('h')).getCost());
		assertEquals("Wrong cost for i", 11, 
				map.getNode(my_vertex_map.get('i')).getCost());
	}
	...
	private void addEdge(final SimpleGraph the_graph, final char the_first, 
			final char the_second, final int the_cost, Map<Character, Vertex> the_nodes) {
