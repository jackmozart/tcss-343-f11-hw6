...
public class DikeGui extends JFrame implements ActionListener{
	...
	private Hashtable<String, Vertex> my_hashTabel;
	
	public DikeGui (){
		super();
		//sets up the window
		this.setTitle("Dijkstra owns AAA");
	...
	public void actionPerformed(ActionEvent the_event) {
		if (the_event.getSource() == my_path) {
			...
			Dikstra dike = new Dikstra(my_graph, my_hashTabel.get(my_startList
					.getSelectedItem()), my_hashTabel.get(my_endList
					.getSelectedItem()));

			my_output.setText(dike.toString());			
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				my_hashTabel = GraphInput.LoadSimpleGraph(my_graph,my_fileChooser.getSelectedFile().getAbsolutePath());
				...
				for (Vertex a_vertex : my_hashTabel.values()) {
					my_startList.addItem(a_vertex.getName());
					my_endList.addItem(a_vertex.getName());
				}
			}
		}
		this.pack();
